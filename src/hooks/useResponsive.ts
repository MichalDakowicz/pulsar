...
// Tailwind's default scale, plus the two ultrawide steps the wall grid needs.
...
// Content column caps. Full-bleed text and forms are unreadable past ~90
// characters, so every desktop screen funnels through one of these.
export const MAX_W = {
  /** The habit wall and the awards grid - wide, but not edge-to-edge. */
  grid: 1800,
  /** Habit detail and stats - two-column reading width. */
  detail: 1180,
  /** Today, settings, the builder, single-column prose. */
  text: 920,
  /** Auth card, dialogs. */
  form: 460,
} as const;
...
/**
 * Width of the element this is attached to, not of the window. Column counts
 * have to be derived from the space a grid actually gets: with an 1800px
 * content cap, window width overshoots and the grid renders too many columns.
 *
